...
import "./starView.css";
import ViewModal from "../components/Eng/Modal_eng";
import plus from "../img/plus2.png";
import FormModal from "../components/Eng/FormModal_eng";
...
import RequestLogin from "../components/Eng/RequestLogin_eng";
...
function StarView() {
  ...
  const findColor = async (_findColor) => {
    ...
    colorOwner = await contract.methods
      ...
      .call();
    if (colorOwner == 0x0) {
      alert("The color code does not exist.");
    } else {
  ...
  const findIndex = async (_index) => {
    ...
    if (_index < 0 || _index >= tokenID) {
      alert("It's a star that doesn't exist!");
    } else {
      contract = new caver.klay.Contract(_abi, address);
